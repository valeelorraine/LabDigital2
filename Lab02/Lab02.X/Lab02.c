...
#include "config_ADC.h"           // Librería para configurar el ADC
#include "LCD.h"                  // Librería de la LCD
...
void __interrupt()isr(void){
    ...
    if(PIR1bits.RCIF == 1){
        ...
        if (RXREC == 43){
            CONTADOR++;            // Incrementar el valor
        }
        if (RXREC == 45){
            CONTADOR--;            // Decrementar el valor 
        }
...
while(1) {
    ...
    if(PIR1bits.TXIF == 1){
        ...
        __delay_ms(150);
        TXREG = cen;               // Enviar palabras a terminal
           }
        __delay_ms(10);
        if(PIR1bits.TXIF == 1){
            TXREG = 46;            // Punto
    ...
    Lcd_Write_Char(un);            // Ingresarle el valor de las unidades
    Lcd_Write_String("  ");
    __delay_us(1000);              // Colocarle un delay para que no loquee
    ...
    if(PIR1bits.TXIF == 1){
        ...
        __delay_ms(10);
        if(PIR1bits.TXIF == 1){
            TXREG = dec;           // Enviar palabras a la terminal
           }
        __delay_ms(10);
        if(PIR1bits.TXIF == 1){
            TXREG = un;            // Enviar palabras a la terminal
           }
        __delay_ms(10);
        if(PIR1bits.TXIF == 1){
            TXREG = 13;            // Enter
...
//ContadorRX
    ...
    Lcd_Write_Char(cen);           // Colocar las centenas
    Lcd_Write_Char(dec);           // Colocar las decenas
    Lcd_Write_Char(un);            // Colocar las unidades
    Lcd_Write_String("  ");
    __delay_ms(100);
    }
}
